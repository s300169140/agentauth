...
function mockProvider(opts: {
  ...
  if (opts.onAuthorize) {
    if (provider.defaultFlow === "device") provider.authorizeDevice = opts.onAuthorize
...
test("requested scope not in cached token triggers fresh authorize", async () => {
  ...
  const provider = mockProvider({
    onAuthorize: async () => {
      authorizeCalls++
      return { accessToken: "tok-broad", scopes: ["read", "write"], provider: "mock" }
    },
  })
  const storage = new MemoryTokenStorage()
  await storage.set("mock:default", {
...
test("authorize() with flow that provider does not support throws", async () => {
  const provider = mockProvider({
    defaultFlow: "device",
    onAuthorize: async () => ({ accessToken: "x", scopes: [], provider: "mock" }),
  })
  const auth = new AgentAuth({
    providers: [provider],
    storage: new MemoryTokenStorage(),
